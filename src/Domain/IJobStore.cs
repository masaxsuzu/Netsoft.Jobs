...
public interface IJobStore
{
    ...
    /// <remarks>
    /// <para>
    /// <b>選ぶ順は登録の古い順だけ。</b><see cref="Job.CreatedAt"/> は登録時に決まって
    /// 二度と動かないので、それがそのまま「列の中の位置」になる ── 停止して再開した Job は
    /// 自分が居た場所へ戻り、先に登録された Job を追い越すことも、後から登録された Job に
    /// 抜かれることも無い。
    /// </para>
    /// <para>
    /// 一度「Registered をすべて先に出す」を入れて外した。再開したものを次に走らせない、
    /// という意図だったが、<b>登録が途切れない限り再開した Job が動き出さない</b>ことと、
    /// 下の保留の規則と重なって<b>止めた本人が、止めていた間に溜まった列に抜かれる</b>ことに
    /// なった。どちらも指示に無い副作用で、利用者の判断で戻している。
    /// </para>
    /// <para>
    /// <b>列を止める状態（<see cref="JobStatusExtensions.BlocksQueue"/>）の Job が 1 件でもあれば、
    /// 待機中が居ても null を返す。</b>保留中があるあいだは実行を進めず、その再開
    /// （またはキャンセル）を待つ、という利用者の決定による。<b>裏を返すと、再開もキャンセルも
    /// されない保留中が 1 件あると、実行は止まったままになる。</b>
    /// </para>
    /// <para>
    /// 止める状態に <see cref="JobStatus.Resuming"/> が入っているのは、
    /// <b>再開の 2 回の書き込みの間に開く窓</b>を塞ぐため。理由は
    /// <see cref="JobStatusExtensions.BlocksQueue"/> に書いてある。
    /// </para>
    /// <para>
